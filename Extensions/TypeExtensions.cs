    ...
    internal static class TypeExtensions
    {
        internal static bool IsCompatibleWith(Type source, Type target)
        {
            ...
            if (nonNullableType1 != source && nonNullableType2 == target)
                return false;
            TypeCode typeCode1 = nonNullableType1.GetTypeInfo().IsEnum ? TypeCode.Object : Type.GetTypeCode(nonNullableType1);
        ...
        internal static MemberInfo FindPropertyOrField(Type type, string memberName, bool staticAccess)
        {
            ...
            {
                MemberInfo[] members = type1.GetTypeInfo().FindMembers(MemberTypes.Field | MemberTypes.Property, bindingAttr,
                    (x, filterCriteria) => x.Name.Equals(filterCriteria), memberName);
                if (members.Length != 0)
                    return members[0];
            }
            return null;
