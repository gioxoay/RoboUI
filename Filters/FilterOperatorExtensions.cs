...
namespace RoboUI.Filters
{
    internal static class FilterOperatorExtensions
    {
        ...
        internal static readonly MethodInfo StringStartsWithMethodInfo = typeof(string).GetMethod("StartsWith",
            new[]
            {
                typeof (string)
            });

        internal static readonly MethodInfo StringEndsWithMethodInfo = typeof(string).GetMethod("EndsWith", new[]
        {
            typeof (string)
        });

        internal static readonly MethodInfo StringCompareMethodInfo = typeof(string).GetMethod("Compare", new[]
        {
            typeof (string),
            typeof (string)
        });

        internal static readonly MethodInfo StringContainsMethodInfo = typeof(string).GetMethod("Contains", new[]
        {
            typeof (string)
        });
        ...
        internal static Expression CreateExpression(this FilterOperator filterOperator, Expression left,
            Expression right, bool liftMemberAccess)
        {
            switch (filterOperator)
            {
                ...
                case FilterOperator.IsContainedInList:
                    return GenerateIsContainedInList(left, right);
        ...
        {
            return
                Expression.Not(GenerateCaseInsensitiveStringMethodCall(StringContainsMethodInfo, left, right,
                    liftMemberAccess));
        }
        ...
        private static Expression GenerateIsContainedInList(Expression left, Expression right)
        {
        ...
        private static Expression GenerateCaseInsensitiveStringMethodCall(MethodInfo methodInfo, Expression left,
            Expression right, bool liftMemberAccess)
        {
            Expression instance = GenerateToLowerCall(left, liftMemberAccess);
            Expression expression = GenerateToLowerCall(right, liftMemberAccess);
            if (methodInfo.IsStatic)
                return Expression.Call(methodInfo, new[]
                {
                    instance,
                    expression
                });
            return Expression.Call(instance, methodInfo, new[]
            {
                expression
            });
        }
