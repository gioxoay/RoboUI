...
namespace RoboUI
{
    ...
    public class RoboUIGridColumn<TModel> : RoboUIGridColumn
    {
        ...
        internal Action<TModel, dynamic> ValueSetter { get; private set; }
        ...
        public void SetValueGetter<TValue>(Func<TModel, TValue> expression)
        {
        ...
        internal void SetValueSetter(Action<TModel, dynamic> action)
        {
            ValueSetter = action;
        }
        ...
        public RoboUIGridColumn<TModel> HasFormatType(Type type)
        {
            TypeFormat = type;
            CustomType = true;
            return this;
        ...
        public RoboUIGridColumn<TModel> RenderAsLink(Func<TModel, string> text, Func<TModel, string> href, bool targetBlank = true)
        {
            if (targetBlank)
            {
                HtmlBuilder = model => string.Format("<a href=\"{0}\" target=\"_blank\">{1}</a>", href(model), WebUtility.HtmlEncode(text(model)));
            }
            else
            {
                HtmlBuilder = model => string.Format("<a href=\"{0}\">{1}</a>", href(model), WebUtility.HtmlEncode(text(model)));
            }
            return this;
        }
